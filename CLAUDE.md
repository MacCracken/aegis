...
daimon (security policy enforcement), argonaut (boot hardening). Aegis is the central security daemon that enforces system-wide security policy.
...
- Security defaults must be restrictive — deny by default, allow explicitly
- All crypto operations belong in sigil — aegis enforces policy, not cryptography
- Firewall integration via nein is feature-gated (`firewall` feature)
- Policy changes must be auditable — every decision logged with context
...
- Do not implement cryptographic primitives — all crypto belongs in sigil
- Do not default to permissive security — always deny-by-default
